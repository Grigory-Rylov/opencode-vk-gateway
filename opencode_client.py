...
class OpenCodeClient:
    ...
    async def get_pending_permissions(self) -> List[dict]:
        ...
        try:
            async with self.session.get(f"{self.base_url}/permission") as resp:
                if resp.status != 200:
                    return []
                return await resp.json()
        except Exception as e:
    ...
    async def send_permission_response(
        ...
        async with self.session.post(url, json=data) as resp:
            if resp.status == 200:
                logger.debug(f"Permission {permission_id} answered: {response}")
                return True
    ...
    async def get_pending_questions(self) -> List[dict]:
        ...
        try:
            async with self.session.get(f"{self.base_url}/question") as resp:
                if resp.status != 200:
                    return []
                return await resp.json()
        except Exception as e:
